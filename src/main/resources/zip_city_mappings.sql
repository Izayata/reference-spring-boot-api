-- Zip code -> city reference data for the GET /v1/zip-codes/{zipCode} lookup endpoint.
-- Source: https://github.com/ferenci-tamas/IrszHnk (IrszHnk.csv, snapshot dated 2026-02-09),
-- which cross-references Magyar Posta's official IRSZ (postal code) file with KSH's
-- Helysegnevtar (settlement gazetteer) -- the two authoritative sources named in
-- docs/BACKEND_ZIP_CITY_LOOKUP.md section 4. Dev-only seed data; see application-dev.yml.
--
-- Generated from 3569 source rows: 3045 unique zip codes inserted,
-- 0 rows skipped (malformed/non-4-digit zip), 0 rows
-- skipped (settlement name outside the existing City validation charset/length rules).
-- Where a zip code covers multiple settlements, the main settlement (no Telepulesresz
-- suffix, then highest population) was picked as the canonical city.
INSERT INTO
    zip_city_mappings(zip_code, city)
VALUES
    ('1007', 'Budapest'),
    ('1011', 'Budapest'),
    ('1012', 'Budapest'),
